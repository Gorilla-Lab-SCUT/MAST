...
from MAST.training.train_pose import train_pose
...
def make_cfg(args):
    ...
    cfg.gs_loss = args.gs_loss
    cfg.icg_loss = args.icg_loss
    ...
    if 'bop-' in args.config:
        from MAST.bop_config import BOP_CONFIG
        from MAST.bop_config import PBR_COARSE, PBR_REFINER
    ...
    if len(args.comment) > 0:
        cfg.run_id = f'{args.config}-{run_comment}-{args.comment}'
    else:
        cfg.run_id = f'{args.config}-{run_comment}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gs_loss', action='store_true')
    parser.add_argument('--icg_loss', action='store_true')
