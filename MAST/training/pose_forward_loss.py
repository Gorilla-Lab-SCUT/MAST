...
from MAST.models.loss_ops import TCO_init_from_boxes, TCO_init_from_boxes_zup_autodepth
...
from MAST.models.loss_ops import (
    ...
    ExplicitInterClassGraphLoss,
    xentropy, cal_gt_dR_vxyz
)
from MAST.models.loss_ops import compute_ADD_L1_loss
from MAST.models.bin_delta_Rt import (
    Rts_to_bin_delta_batch,
    bin_delta_to_Rs_batch_by_idx, 
    bin_delta_to_ts_batch_by_idx,
    sample_rotations_60, grid_xyz
)
...
def h_pose(model, mesh_db, data, meters,
    ...
    if input_generator == 'fixed':
        ...
        TCO_init = add_noise(TCO_possible_gt[:, 0], euler_deg_std=[15, 15, 15], trans_std=[0.01, 0.01, 0.1])  # original z std: 0.05
    ...
    if cfg.icg_loss:
        ...
        loss += loss_icg
    return loss, iter_outputs['z_feat'], torch.max(zb_gt, dim=-1)[1]
