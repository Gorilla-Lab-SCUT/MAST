...
from MAST.lib3d.camera_geometry import get_K_crop_resize
from .dataset_utils import make_detections_from_segmentation, crop_to_aspect_ratio
...
class BackgroundAugmentation:
    ...
    def __call__(self, im, mask, obs):
        if random.random() <= self.p:
            ...
            im_bg = to_pil(im_bg)
            im_bg = (transforms.ToTensor()(im_bg.resize((w, h))) * 255).to(torch.uint8).permute(1, 2, 0)
            mask_bg = mask == 0
