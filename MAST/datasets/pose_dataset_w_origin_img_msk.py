...
from MAST.lib3d import invert_T
...
class PoseData:
    ...
    def pin_memory(self):
        self.images_origin = self.images_origin.pin_memory()
...
class PoseDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        self.background_augmentations = VOCBackgroundAugmentation(
            voc_root=LOCAL_DATA_DIR / 'VOCdevkit/VOC2012', p=bg_aug_p)
    ...
    def collate_fn(self, batch):
        ...
        for k in batch[0].__annotations__:
            ...
            if k in ('images_origin', 'mask', 'images', 'bboxes', 'TCO', 'K'):
    ...
    def get_data(self, idx):
        ...
        rgb, mask, rgb_origin = to_torch_uint8(rgb), to_torch_uint8(mask), to_torch_uint8(rgb_origin)
        mask_uniqs = set(np.unique(mask))
        objects_visible = []
        for obj in state['objects']:
            add = False
            if obj['id_in_segm'] in mask_uniqs and np.all(np.array(obj['bbox']) >= 0) and obj['visib_fract'] > 0.05:
                add = True
            # if obj['label'] == 'obj_000001':
            #     add = True
            # else:
            #     add = False
        ...
        rgb_origin = torch.as_tensor(rgb_origin).permute(2, 0, 1).to(torch.uint8)
        ...
        data = PoseData(
            images_origin=np.asarray(rgb_origin),
