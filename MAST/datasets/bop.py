...
from MAST.config import MEMORY, BOP_TOOLKIT_DIR
...
class BOPDataset:
    '''single_obj_name: if exists, the dataset returns single object
    '''
    def __init__(self, ds_dir, split='train', load_depth=False, single_obj_name: str = None):
        ds_dir = Path(ds_dir)
        self.ds_dir = ds_dir
        ...
        self.single_obj_name = single_obj_name
    ...
    def __getitem__(self, frame_id):
        ...
        if 'scene_gt_info' in self.annotations[scene_id_str]:
            ...
            for n in range(n_objects):
                ...
                if self.single_obj_name is not None:
                    if self.single_obj_name != name: continue
