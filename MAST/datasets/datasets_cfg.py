...
from MAST.config import LOCAL_DATA_DIR, ASSET_DIR, BOP_DS_DIR
...
def make_scene_dataset(ds_name, n_frames=None):
    ...
    # BOP challenge
        ...
        ds = BOPDataset(ds_dir, split='test_bop')
        ds = keep_bop19(ds)
...
def make_object_dataset(ds_name):
    ...
    elif ds_name == 'hb.eval':
        ds = BOPObjectDataset(BOP_DS_DIR / 'hb/models_eval')
    elif ds_name == 'icbin':
    ...
    elif ds_name =='lm.eval':
        ds = BOPObjectDataset(BOP_DS_DIR / 'lm/models_eval')
    elif ds_name == 'tudl':
