...
from MAST.utils.timer import Timer
...
class CoarseRefinePosePredictor(torch.nn.Module):
    ...
    @torch.no_grad()
    def batched_model_predictions(self, model, images, K, obj_data, n_iterations=1, transes=None):
        timer = Timer()
        timer.start()
        ...
        for (batch_ids, ) in dl:
            ...
            outputs = model(images=images_, K=K_, TCO=TCO_input,
                            n_iterations=n_iterations, labels=labels, transes=transes)
            timer.pause()
    ...
    def get_predictions(self, images, K,
        ...
        if data_TCO_init is None:
            ...
            coarse_preds = self.batched_model_predictions(self.coarse_model,
                                                          ...
                                                          n_iterations=n_coarse_iterations,transes=transes)
