...
def deepim_crops_robust(images, obs_boxes, K, TCO_pred, O_vertices, output_size=None, lamb=1.4, mask=None):
    """
    adjust bbox according to the input (gt)bbox and input TCO
    NOTE: cosypose does not use input (gt)bbox,
    thus this function slightly enlarged the boxes_rend obtained from TCO
    """
    batch_size, _, h, w = images.shape
    device = images.device
    if output_size is None:
    ...
    rend_boxes = boxes_from_uv(uv)
    rend_center_uv = project_points_robust(torch.zeros(batch_size, 1, 3).to(device), K, TCO_pred)  # centre points of rend_boxes
    boxes = deepim_boxes(rend_center_uv, obs_boxes, rend_boxes, im_size=(h, w), lamb=lamb)
    boxes = torch.cat((torch.arange(batch_size).unsqueeze(1).to(device).float(), boxes), dim=1)
    crops = torchvision.ops.roi_align(images, boxes, output_size=output_size, sampling_ratio=4)
    if mask is not None:
        mask_crops = torchvision.ops.roi_align(mask.unsqueeze(1), boxes, output_size=output_size, sampling_ratio=4)
        return boxes[:, 1:], crops, mask_crops.to(torch.int).squeeze(1)
    else:
        return boxes[:, 1:], crops
